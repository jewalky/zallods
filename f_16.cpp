...
Sprite_16::Sprite_16(String filename)
{
    if(!rt_main)
        C_Fatal("Sprite_16::Sprite_16: Couldn't load \"%s\" (video not initialized yet).\n", filename.c_str());

    File f;
    if(!f.open(filename) || !f.length())
        C_Fatal("Sprite_16::Sprite_16: Couldn't open \"%s\".\n", filename.c_str());
    ...
    for(uint32_t i = 0; i < cnt; i++)
    {
        ...
        if(!isz)
            C_Fatal("Sprite_16::Sprite_16: Size of frame %u is NULL.\n", i);
        ...
        while(pos < isz)
        {
            ...
            else if(fbyte)
            {
                ...
                {
                    uint16_t color22 = (pixels[i] & 0x0F);
                    color22 |= (color22 << 4);
                    color22 <<= 8;

                    *upixels++ = color22;

                    if(i != fbyte-1 || (pixels[fbyte-1] & 0xF0))
                    {
                        color22 = (pixels[i] & 0xF0);
                        color22 |= (color22 >> 4);
                        color22 <<= 8;
                        *upixels++ = color22;
                    }
...
void Sprite_16::display(int16_t x, int16_t y, uint32_t num)
{
    displayColored(x, y, num, 255, 255, 255, 255);
}

void Sprite_16::displayColored(int16_t x, int16_t y, uint32_t num, uint8_t cr, uint8_t cg, uint8_t cb, uint8_t ca)
{
